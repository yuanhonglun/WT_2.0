...
def _features_to_dicts(features: list[Feature]) -> list[dict]:
    ...
    for f in features:
        d = {
            ...
            # PR-D: isotope / adduct cluster labels (each copy is an
            # independent counted feature; must survive save -> reload).
            "isotope_index": f.isotope_index,
...
def _dicts_to_features(dicts: list[dict]) -> list[Feature]:
    ...
    for d in dicts:
        f = Feature(
            ...
            # PR-D labels; .get defaults keep old project files loadable.
            isotope_index=d.get("isotope_index", 0),
