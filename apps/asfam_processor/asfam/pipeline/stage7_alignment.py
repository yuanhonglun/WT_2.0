...
# W11 修复 1: 直接走 metabo_core 的 cosine, 与 DDA stage_align (经
# metabo_core.alignment.replicates) 和 GC-MS stage_align 保持同一调用源,
# 避免 ``asfam.core.similarity`` 的 re-export 间接跳转误导读者。
...
# W11 修复 2: 共享的"参考样质量分"helper, ASFAM/DDA/GC-MS 用同一公式。
from metabo_core.alignment import reference_replicate_quality
...
def run_stage7(
    ...
    # W11 修复 2: 复合排序 (n_passes_sn3, mean_sn), 取代旧的 "特征最多者
    # 即参考"。低 S/N replicate 会被自动降权, 真信号不会被迫匹去伪信号。
    ref_id = max(
        rep_ids,
    ...
    for i, aln in enumerate(aligned):
        ...
        feature = Feature(
            ...
            gaussian_similarity=aggregate_feature_gaussian(
                ref_feat.ms1_gaussian, ref_feat.ms2_gaussian,
            ),
            ...
            isotope_index=ref_feat.isotope_index,
            isotope_group_id=ref_feat.isotope_group_id,
            adduct_group_id=ref_feat.adduct_group_id,
        )
